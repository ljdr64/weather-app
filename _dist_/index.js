...
let converseKelvinCelsius = (temp) => {
  return Math.round(temp - constKelvin);
};
  ...
  window
    ...
    .then((responseJson) => {
      ...
      mainTemp.textContent =
        converseKelvinCelsius(responseJson.main.temp) + "°";
      ...
      const mainTempMax = document.querySelector(
        `.main-temp-max-${cityNameL1}`
      );
      mainTempMax.textContent =
        "Max: " + converseKelvinCelsius(responseJson.main.temp_max) + "°";

      const mainTempMin = document.querySelector(
        `.main-temp-min-${cityNameL1}`
      );
      mainTempMin.textContent =
        "Min: " + converseKelvinCelsius(responseJson.main.temp_min) + "°";

      const description = document.querySelector(`.description-${cityNameL1}`);
      description.textContent =
        responseJson.weather[0].description.toUpperCase();
    });
};
...
let accionSearchCity = (event) => {
  // <div class="border-2 border-black text-lg p-4 cards bg-blue-500 w-52 m-2.5 rounded-2xl">
  //         <h1 class="border-2 border-black bg-gray-300 bg-white rounded-md city-name-${searchedCityName}"></h1>
  ...
  if (inputSearch.value !== "") {
    ...
    h1.className = `border-2 border-black bg-gray-300 bg-white rounded-md city-name-${searchedCityName}`;
    ...
    container.className =
      "border-2 border-black text-lg p-4 cards bg-blue-500 w-52 m-2.5 shadow-2xl rounded-2xl";
    container.id = `count-${count}`;

    searchedCityName =
      searchedCityName[0].toUpperCase() +
      searchedCityName.slice(1).toLowerCase().split("-").join(" ");
